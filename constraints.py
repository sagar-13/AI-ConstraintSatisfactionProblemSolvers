...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        self.i = i
        self.j = j
        for valuei in qi.domain():
            for valuej in qj.domain():
                possibleTup = (i, j)
                if not self.queensCheck(valuei, valuej):
                    continue
                else:
                    sat.append([valuei, valuej])
    

        TableConstraint.__init__(self, name, [qi, qj], sat)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            if var.isAssigned():
                if var.getValue() == required_value:
                    count +=1
        
        
        return count >= lower_bound and count <= upper_bound
    ...
    def hasSupport(self, var, val):
        ...
        def PartialCheck(l):
            ...
            for i in  vals:
                if i == self._required:
                    count +=1
            return count <= self._ub  #only difference is this line
        def FinalCheck(l):
            vals = [val for (var, val) in l]
            count = 0
            for i in  vals:
                if i == self._required:
                    count +=1
            return (count >= self._lb and count <= self._ub) # full check here


        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], FinalCheck, PartialCheck)
        return x
